...
class NDSwinClassifier(nn.Module):
    ...
    def setup(self) -> None:
        ...
        current_grid = tuple(
            int(np.ceil(res / patch)) for res, patch in zip(self.resolution, self.patch_size)
        )
        ...
        total_blocks = int(np.sum(np.array(self.depth)))
        if self.drop_path_rate > 0.0:
            drop_schedule = np.linspace(0.0, self.drop_path_rate, total_blocks, dtype=np.float32)
        else:
            drop_schedule = np.zeros((total_blocks,), dtype=np.float32)
        ...
        for stage_depth, heads in zip(self.depth, self.num_heads):
            ...
            current_grid = tuple(
                int(np.ceil(dim / 2)) if use else dim
