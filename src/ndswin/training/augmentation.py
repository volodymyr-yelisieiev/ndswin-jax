...
def create_augmentation_pipeline(
    ...
    Args:
        ...
        skip_normalize: If True, skip adding Normalize transform (useful when
            the data loader already normalizes before calling the pipeline).
    ...
    if is_training and hasattr(config, "augmentation") and config.augmentation:
        ...
        if hasattr(config, "random_crop") and config.random_crop:
            ...
            # If image_size includes channels (e.g. (3,32,32)), strip the channel dim
            if len(img_size) > 2 and img_size[0] == in_ch:
                spatial_size = img_size[1:]
            else:
                spatial_size = img_size
            transforms.append(RandomCrop(spatial_size, padding=4))
    ...
    # Normalization (skip if data loader already normalizes)
    if not skip_normalize and hasattr(config, "normalize") and config.normalize:
