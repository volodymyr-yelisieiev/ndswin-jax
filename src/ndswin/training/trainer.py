...
class TensorBoardScalarWriter:
    ...
    def add_scalar(self, tag: str, value: MetricValue, step: int) -> None:
...
class Trainer:
    ...
    def __init__(
        ...
        if use_tensorboard and checkpoint_dir is not None:
            self.tb_writer = TensorBoardScalarWriter(log_dir=str(checkpoint_dir))
    ...
    def train_epoch(
        ...
        for i, batch in enumerate(train_loader):
            ...
            # Pad batch if needed for multi-GPU sharding
            batch, real_bs = self._pad_batch(batch)

            # Distribute batch to all devices
            batch["image"] = jax.device_put(batch["image"], self.dp_sharding)
            batch["label"] = jax.device_put(batch["label"], self.dp_sharding)
            ...
            # Update tracking (only count real samples, not padding)
            tracker.update(metrics, real_bs)
            self.step += 1
    ...
    def _pad_batch(self, batch: Batch) -> tuple[Batch, int]:
        """Pad batch so its size is divisible by num_devices.
        ...
        if real_bs % self.num_devices != 0:
            ...
            # Repeat the last sample to fill the gap
            img_pad = jnp.repeat(batch["image"][-1:], pad_n, axis=0)
            lbl_pad = jnp.repeat(batch["label"][-1:], pad_n, axis=0)
            batch = {
        ...
        for epoch in range(num_epochs):
            ...
            if val_loader is not None:
                ...
                if self.task == "segmentation":
                    ...
                    v_m = val_metrics.get("dice", val_metrics.get("val_dice", 0.0))
                    m_name = "Dice"
                else:
                    t_m = train_metrics.get("accuracy", train_metrics.get("train_accuracy", 0.0))
            ...
            if self.tb_writer is not None:
                for k, v in train_metrics.items():
                    hist_key = k if k.startswith("train_") else f"train_{k}"
                    self.tb_writer.add_scalar(hist_key, v, epoch + 1)
                if val_loader is not None:
                    for k, v in val_metrics.items():
                        hist_key = k if k.startswith("val_") else f"val_{k}"
                        self.tb_writer.add_scalar(hist_key, v, epoch + 1)
