...
def validate_dataset_contract(
    ...
    *,
    ...
    if require_validation_split:
        ...
        if dataset_name in ARRAY_FOLDER_DATASETS | {
            "medseg_msd",
            "medseg",
            "medseg_brain_tumour",
        }:
...
class VolumeFolderDataLoader(DataLoader):
    """Generic N-D array classification folder data loader.
    ...
    Each sample file should contain a numpy array of shape ``(*spatial)`` for
    single-channel data, or ``(C, *spatial)`` where ``C`` equals
    ``in_channels``. Normalization is applied per channel using ``mean`` and
    ``std`` from configuration.
    """
    ...
    def _load_file(self, path: str) -> np.ndarray:
        ...
        arr = np.asarray(arr)
        target_dims = len(self.image_size)

        if arr.ndim == target_dims:
            if self.in_channels != 1:
                raise ValueError(
                    f"File {path} has no channel axis but expected {self.in_channels} channels"
                )
            arr = np.expand_dims(arr, 0)
        elif arr.ndim == target_dims + 1:
            if arr.shape[0] == self.in_channels:
                pass
            elif arr.shape[-1] == self.in_channels:
                arr = np.moveaxis(arr, -1, 0)
            else:
                raise ValueError(
                    f"File {path} has shape {arr.shape}; expected channel-first "
                    f"({self.in_channels}, *spatial) or channel-last (*spatial, {self.in_channels})"
                )
        else:
            raise ValueError(
                f"Unsupported array shape {arr.shape} for N-D classification file: {path}"
            )

        # If sample spatial differs from expected image_size, allow resize by pad/crop
        spatial = arr.shape[1:]
        target_spatial = tuple(self.image_size)
        if len(spatial) != len(target_spatial):
            raise ValueError(
                f"File {path} has {len(spatial)} spatial dimensions but expected "
                f"{len(target_spatial)} from image_size={target_spatial}"
            )
...
def create_data_loader(
    ...
    internal_norm = ds in {"cifar10", "cifar100"} | ARRAY_FOLDER_DATASETS
    ...
    if ds == "cifar10":
        ...
        try:
            ...
            if "WebDataset" in msg or "TAR archives" in msg or "webdataset" in msg.lower():
                print(
                    f"Hugging Face dataset {hf_id} appears to be packaged as WebDataset "
                    "and failed to load via `datasets`. Falling back to `ndswin fetch-data` "
                    f"and using the local NPZ loader. Error: {e}"
                )
