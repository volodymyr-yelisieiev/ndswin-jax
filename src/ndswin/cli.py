...
def run_train_command(args: argparse.Namespace) -> int:
    ...
    if checkpoint_manager is not None and trainer_state is not None:
        checkpoint_metrics = {
            key: float(value)
        ...
        best_checkpoint_path = checkpoint_manager.save_named(
            ...
            epoch=int(trainer.best_epoch or getattr(trainer, "epoch", 0)),
            metrics=checkpoint_metrics,
            batch_stats=getattr(trainer_state, "batch_stats", None),
            optimizer_state=getattr(trainer_state, "opt_state", None),
        )
        train_logger.info("Best/restored checkpoint saved to: %s", best_checkpoint_path)
    ...
    metrics_data = {
        ...
        "artifact_paths": {
            ...
            "config_copy": str(checkpoint_dir.parent / "config.json"),
...
def build_command(job: dict[str, Any], python_exe: str) -> list[str]:
    ...
    if job_type == "fetch-data":
        hf_id = job.get("hf_id") or job.get("hf_dataset")
        outdir = job.get("outdir") or job.get("data_dir")
        if not hf_id:
            raise ValueError(f"fetch-data job requires 'hf_id' field: {job}")
        if not outdir:
            raise ValueError(f"fetch-data job requires 'outdir' field: {job}")
...
def run_queue_command(args: argparse.Namespace) -> int:
    ...
    results_path = (
        ...
        else Path("logs") / f"queue_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def _default_export_outdir(metrics_path: Path, metrics_data: dict[str, Any]) -> Path:
    config = metrics_data.get("config", {})
    if isinstance(config, dict):
        name = str(config.get("name", metrics_path.parent.parent.name))
    else:
        name = metrics_path.parent.parent.name
    safe_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", name).strip("_") or "ndswin_model"
    return Path("exports") / safe_name


def run_export_weights_command(args: argparse.Namespace) -> int:
    """Create a local Hugging Face-ready weights bundle without pushing it."""
    run_dir = Path(args.run_dir) if args.run_dir else None
    metrics_path = Path(args.metrics) if args.metrics else None
    if metrics_path is None:
        if run_dir is None:
            logger.error("Either --run-dir or --metrics is required.")
            return 1
        metrics_path = run_dir / "checkpoints" / "metrics.json"
    if not metrics_path.exists():
        logger.error("Metrics file not found: %s", metrics_path)
        return 1

    metrics_data = cast(dict[str, Any], json.loads(metrics_path.read_text()))
    checkpoint_path = Path(
        args.checkpoint or metrics_data.get("best_checkpoint") or metrics_path.parent / "best.npz"
    )
    config_path = Path(
        args.config
        or metrics_data.get("config_path")
        or (
            run_dir / "config.json"
    ...
    outdir = (
        Path(args.outdir) if args.outdir else _default_export_outdir(metrics_path, metrics_data)
    )
    outdir.mkdir(parents=True, exist_ok=True)

    weights_dest = outdir / "weights.npz"
    ...
    (outdir / "export_manifest.json").write_text(json.dumps(manifest, indent=2))
    print(f"Export bundle written to: {outdir}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    queue_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional explicit path for queue JSON results",
    )
    queue_parser.add_argument(
    ...
    export_parser = subparsers.add_parser(
        "export-weights",
        help="Create a local Hugging Face-ready bundle from a completed run",
    )
    export_parser.add_argument(
        "--run-dir",
        type=str,
        default=None,
        help="Run directory containing config.json and checkpoints/metrics.json",
    )
    export_parser.add_argument(
        "--metrics", type=str, default=None, help="Explicit metrics.json path"
    )
    export_parser.add_argument(
        "--checkpoint", type=str, default=None, help="Explicit best checkpoint path"
    )
    export_parser.add_argument("--config", type=str, default=None, help="Explicit config path")
    export_parser.add_argument("--outdir", type=str, default=None, help="Export directory")
    export_parser.add_argument(
        "--hf-repo-id",
        type=str,
        default=None,
        help="Target Hugging Face repo id to record in the model card",
    )
    export_parser.set_defaults(func=run_export_weights_command)
