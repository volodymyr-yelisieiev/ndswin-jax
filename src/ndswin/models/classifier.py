...
class SwinForSegmentation(nn.Module):
    ...
    def __call__(
        ...
        for i in range(len(features) - 2, -1, -1):
            ...
            # Refine channels at each spatial location without relying on
            # dimension-specific convolution kernels.
            decoded = nn.Dense(skip_channels, name=f"refine_{i}")(decoded)
        ...
        # Classification head over channels at each spatial location.
        logits = nn.Dense(self.num_classes, name="seg_head")(decoded)
