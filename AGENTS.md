...
The current Makefile can also create/update that project-local environment:

- The **documented direct Conda workflow** is:
  ```bash
...
- The **Makefile bootstrap workflow** is:
  ```bash
  make env
  conda activate ./Environment/ndswin-jax
  ```
  If Conda/Mamba is missing, `make env` bootstraps a local Miniforge under `.tools/` before creating `Environment/ndswin-jax`.

So in practice, agents should run `make env` if the project-local environment is missing, then use `make`. For individual tools, the Makefile resolves executables from `Environment/ndswin-jax/bin` first and falls back to the active shell `PATH` only when that prefix is absent.
...
- The current queue runner supports `fetch-data`, `train`, `sweep`, and `auto-sweep` jobs.
...
- `benchmark-tmux` is the preferred wrapper for unattended final 2D+3D practical-work runs. `queue-tmux` remains useful for custom queues.
