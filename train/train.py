...
def create_augmentation_transform(
    ...
    # Mixup (applied at batch level, not per-sample)
    mixup_transform = None
    mixup_alpha = aug_cfg.get("mixup_alpha", 0.0)
    if mixup_alpha > 0:
        mixup_transform = Mixup(alpha=mixup_alpha)
...
def setup_output_dirs(
    ...
    # Create checkpoint directory with config stamp
    checkpoint_base = Path("checkpoints")
    checkpoint_dir = checkpoint_base / stamp
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    # Copy config to checkpoint directory for reference
    config_copy_path = checkpoint_dir / "config.json"
...
def main() -> None:
    ...
    checkpoint_dir, log_file = setup_output_dirs(exp_config, str(config_path))
    ...
    try:
        ...
        metrics_data = {
            ...
            "final_metrics": {
                ...
                "accuracy": float(final_metrics["accuracy"]) if val_loader else None,
                "top5_accuracy": float(final_metrics["top5_accuracy"]) if val_loader else None,
            },
