...
def run_trial(trial_idx: int, exp: ExperimentConfig, out_dir: Path, dry_run: bool = False) -> Dict[str, Any]:
    ...
    if task == "segmentation":
        model = SwinForSegmentation(
            config=exp.model,
            num_classes=exp.model.num_classes or 2,
        )
    else:
    ...
    mixup_alpha = getattr(exp.data, "mixup_alpha", 0.0)
    cutmix_alpha = getattr(exp.data, "cutmix_alpha", 0.0)
    if mixup_alpha > 0 or cutmix_alpha > 0:
        from ndswin.training.augmentation import MixupOrCutmix
        num_classes = exp.model.num_classes or 100
    ...
    try:
        ...
        # Try validation split first, fallback to test
        try:
            val_loader = create_data_loader(exp.data, split="validation", batch_size=exp.training.batch_size, pad_to=pad_to)
        except Exception:
            val_loader = create_data_loader(exp.data, split="test", batch_size=exp.training.batch_size, pad_to=pad_to)
    except Exception as e:
        # Record trial failure and continue with other trials
        err_msg = str(e)
        print(f"Trial {trial_idx} failed during data loading: {err_msg}")
        return {"trial": trial_idx, "status": "error", "message": err_msg, "dataset": dataset_name, "stamp": stamp}
    ...
    try:
        ...
        print(f"Trial {trial_idx} interrupted by user")
    except Exception as exc:
        print(f"Trial {trial_idx} training error: {exc}")
    finally:
    ...
    try:
        ...
        print(f"Trial {trial_idx} evaluation failed: {eval_err}")
        val_metrics = {}
    ...
    if getattr(exp.data, "task", "classification") == "segmentation":
        ...
        print(f"Trial {trial_idx} done - val_accuracy={results['val_accuracy']:.4f} (elapsed {elapsed:.1f}s)")
