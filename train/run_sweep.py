...
def run_trial(trial_idx: int, exp: ExperimentConfig, out_dir: Path, dry_run: bool = False, sweep_config: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if dry_run:
        ...
        return {"trial": trial_idx, "status": "dry", "config_path": str(config_path), "dataset": dataset_name, "stamp": stamp}
    ...
    train_cmd = [
        ...
        "--epochs", str(exp.training.epochs),
        "--stamp", f"{stamp}_trial_{trial_idx:03d}",
    ...
    try:
        ...
        with open(log_file_path, "w") as log_file:
            subprocess.run(train_cmd, stdout=log_file, stderr=subprocess.STDOUT, check=True, env=env, timeout=1800)
        status = "success"
    ...
    metrics_file = Path("outputs") / dataset_name / f"{stamp}_trial_{trial_idx:03d}" / "checkpoints" / "metrics.json"
    ...
    if status == "success" and metrics_file.exists():
        try:
            ...
            if getattr(exp.data, "task", "classification") == "segmentation":
                ...
                logger.info("Trial %d done - val_dice=%.4f (elapsed %.1fs)", trial_idx, results['val_dice'], elapsed)
            else:
                ...
                logger.info("Trial %d done - val_accuracy=%.4f, val_top5=%.4f (elapsed %.1fs)",
                             trial_idx, results['val_accuracy'], results['val_top5_accuracy'], elapsed)
        except Exception as e:
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                if "embed_dim" in msg and "divisible" in msg and "num_heads" in msg:
                    ...
                    if nearest <= 0:
                        nearest = num_heads0
                    logger.debug("Adjusting sampled model.embed_dim %s -> nearest valid %s (multiple of %s)", base_embed, nearest, num_heads0)
                    sampled["model.embed_dim"] = nearest
                    attempt += 1
                    continue
                # Otherwise, resample (some parameters might cause failures like patch/window mismatch)
                attempt += 1
                logger.debug("Sampled invalid config on attempt %d/%d: %s; resampling...", attempt, max_attempts, e)
                continue
