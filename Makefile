.PHONY: help env install install-dev test lint format type-check docs clean clean-runs clean-all build check all dev train train-bg train-tmux list-configs sweep sweep-tmux auto-sweep auto-sweep-tmux queue queue-tmux benchmark benchmark-tmux fetch-data status archive-results optimize tensorboard stop-all show-best validate backup clean-logs print-env-resolution
...
BENCHMARK_QUEUE ?= configs/queues/benchmark_2d_then_3d.yaml
TRAIN_EPOCHS  ?= 100
SWEEP_TRIALS  ?=
...
FETCH_LIMIT   ?=
TRIAL_TIMEOUT ?= 7200
ENV_PREFIX    ?= $(if $(strip $(CONDA_PREFIX_DIR)),$(CONDA_PREFIX_DIR),Environment/ndswin-jax)
MINIFORGE_DIR ?= .tools/miniforge3
...
CONDA_BIN_DIR    := $(if $(strip $(CONDA_PREFIX_DIR)),$(if $(wildcard $(CONDA_PREFIX_DIR)/bin/python),$(CONDA_PREFIX_DIR)/bin,),)
...
HELP_ENV_PREFIX := $(if $(strip $(CONDA_PREFIX_DIR)),$(CONDA_PREFIX_DIR),$(ENV_PREFIX))
...
help:
	...
	@echo ""
	@echo "★ Quick Start:"
	@echo "  conda env create --prefix ./$(HELP_ENV_PREFIX) -f environment.yml"
	@echo "  conda activate ./$(HELP_ENV_PREFIX)"
	@echo "  make env          Create/update the project-local environment"
	...
	@echo "  benchmark          Canonical 2D→3D autonomous benchmark queue"
	@echo "  benchmark-tmux     Detached tmux wrapper for benchmark"
	@echo ""
	@echo "Data:"
	...
	@echo "  make benchmark-tmux                                   # detached final benchmark"
...
env:
	...
	if [ -x "$$PREFIX/bin/python" ]; then \
		echo "Environment already exists: $$PREFIX"; \
	else \
		CONDA_EXE=""; \
		if command -v conda >/dev/null 2>&1; then CONDA_EXE="$$(command -v conda)"; fi; \
		if [ -z "$$CONDA_EXE" ] && command -v mamba >/dev/null 2>&1; then CONDA_EXE="$$(command -v mamba)"; fi; \
		if [ -z "$$CONDA_EXE" ] && command -v micromamba >/dev/null 2>&1; then CONDA_EXE="$$(command -v micromamba)"; fi; \
		if [ -z "$$CONDA_EXE" ] && [ -x "$(MINIFORGE_DIR)/bin/conda" ]; then CONDA_EXE="$(MINIFORGE_DIR)/bin/conda"; fi; \
		if [ -z "$$CONDA_EXE" ]; then \
			echo "No conda/mamba/micromamba found; bootstrapping local Miniforge under $(MINIFORGE_DIR)"; \
			mkdir -p .tools; \
			curl -L --fail -o .tools/Miniforge3-Linux-x86_64.sh https://github.com/conda-forge/miniforge/releases/latest/download/Miniforge3-Linux-x86_64.sh; \
			bash .tools/Miniforge3-Linux-x86_64.sh -b -p "$(MINIFORGE_DIR)"; \
			CONDA_EXE="$(MINIFORGE_DIR)/bin/conda"; \
		fi; \
		echo "Creating environment at $$PREFIX using $$CONDA_EXE"; \
		"$$CONDA_EXE" env create --prefix "$$PREFIX" -f environment.yml || "$$CONDA_EXE" env update --prefix "$$PREFIX" -f environment.yml; \
	fi; \
	"$$PREFIX/bin/python" -m pip install --upgrade pip; \
	"$$PREFIX/bin/python" -m pip install -e ".[dev,training,gpu]"; \
	echo "Environment ready: $$PREFIX"; \
	"$$PREFIX/bin/python" -c 'import sys; print(sys.version); import jax; print("jax", jax.__version__); print("devices", jax.devices())'
...
stop-all:
	...
	@tmux ls 2>/dev/null | grep -E "^(train_|sweep_|autosweep_|queue_|benchmark_)" | cut -d: -f1 | while read s; do \
		echo "  Killing: $$s"; \
...
benchmark:
	@if [ ! -f "$(BENCHMARK_QUEUE)" ]; then echo "Error: Benchmark queue not found: $(BENCHMARK_QUEUE)"; exit 1; fi
	@echo "Running benchmark queue: $(BENCHMARK_QUEUE)"
	$(HF_ENV) $(NDSWIN) queue --queue $(BENCHMARK_QUEUE) --skip-completed --stop-on-error $(EXTRA_ARGS)

benchmark-tmux:
	@if [ ! -f "$(BENCHMARK_QUEUE)" ]; then echo "Error: Benchmark queue not found: $(BENCHMARK_QUEUE)"; exit 1; fi
	@STAMP=$$(date +%Y%m%d_%H%M%S); \
	SESSION="benchmark_$${STAMP}"; LOG_DIR="logs/tmux/benchmark"; LOG="$${LOG_DIR}/benchmark_$${STAMP}.log"; mkdir -p "$${LOG_DIR}"; \
	echo "Starting benchmark in tmux: $${SESSION}"; \
	tmux new -d -s "$${SESSION}" "cd $(shell pwd) && $(HF_ENV) $(NDSWIN) queue --queue $(BENCHMARK_QUEUE) --skip-completed --stop-on-error $(EXTRA_ARGS) 2>&1 | tee '$${LOG}'; exec bash"; \
	echo "Attach: tmux attach -t $${SESSION}"; echo "Log: $${LOG}"
