...
train-bg:
	...
	nohup $(PYTHON) train/train.py --config $(CONFIG) $(TRAIN_ARGS) > "$${LOG_FILE}" 2>&1 & \
	echo "PID: $$!"; \
...
train-tmux:
	...
	@CONFIG_NAME=$$(basename $(CONFIG) .json); \
	TIMESTAMP=$$(date +%Y%m%d_%H%M%S); \
	SESSION_NAME="train_$${CONFIG_NAME}_$${TIMESTAMP}"; \
	LOG_FILE="logs/$${CONFIG_NAME}_$${TIMESTAMP}.log"; \
	mkdir -p logs; \
	echo "Starting tmux training session: $${SESSION_NAME}"; \
	echo "Config: $(CONFIG)"; \
	echo "Log file: $${LOG_FILE}"; \
	tmux new -d -s "$${SESSION_NAME}" "cd $(shell pwd) && $(PYTHON) train/train.py --config $(CONFIG) $(TRAIN_ARGS) 2>&1 | tee '$${LOG_FILE}'; exec bash"; \
	echo ""; \
	echo "Commands:"; \
