...
def test_cli_help_lists_subcommands():
    ...
    assert "export-weights" in result.stdout
...
def test_run_train_command_persists_best_metric_metadata(tmp_path: Path, monkeypatch):
    ...
    class FakeTrainer:
        def __init__(self, *args, **kwargs):
            ...
            self.checkpoint_manager = CheckpointManager(kwargs["checkpoint_dir"])
    ...
    assert metrics["best_checkpoint"].endswith("best.npz")
    assert Path(metrics["best_checkpoint"]).exists()
    assert metrics["artifact_paths"]["metrics_file"].endswith("metrics.json")


def test_export_weights_creates_hf_ready_bundle(tmp_path: Path):
    import ndswin.cli as cli

    run_dir = tmp_path / "outputs" / "demo" / "run_001"
    ...
    args = SimpleNamespace(
        ...
        config=None,
        outdir=str(outdir),
        hf_repo_id="volodymyr-yelisieiev/ndswin-demo",
    )

    assert cli.run_export_weights_command(args) == 0
