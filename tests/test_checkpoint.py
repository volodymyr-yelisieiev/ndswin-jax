...
def test_checkpoint_manager_save_named_best_checkpoint():
    """Named checkpoints should support the final restored-best artifact."""
    with tempfile.TemporaryDirectory() as tmp_dir:
        manager = CheckpointManager(directory=tmp_dir, max_to_keep=1)
        params = {"layer": {"w": jnp.ones((2, 2))}}

        path = manager.save_named(
            "best.npz",
            params=params,
            step=12,
            epoch=3,
        ...
        assert path.endswith("best.npz")
        assert os.path.exists(path)
        loaded = manager.load(path=path)
        assert loaded["step"] == 12
        assert loaded["epoch"] == 3
        assert loaded["metrics"]["val_accuracy"] == 0.75
        assert jnp.allclose(loaded["params"]["layer"]["w"], params["layer"]["w"])
