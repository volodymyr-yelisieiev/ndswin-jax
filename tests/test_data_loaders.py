...
def test_array_folder_loader_supports_nd_single_channel_arrays(tmp_path):
    """Local classification folders should work for N-D arrays, not only 3D volumes."""
    class_dir = tmp_path / "train" / "class_a"
    class_dir.mkdir(parents=True)
    np.save(class_dir / "sample.npy", np.ones((4, 4), dtype=np.float32))
    ...
    assert batch["image"].shape == (1, 1, 6, 6)
    assert batch["label"].tolist() == [0]


def test_array_folder_loader_supports_channel_first_npz(tmp_path):
    """NPZ samples with an image key may provide explicit channel-first tensors."""
    class_dir = tmp_path / "train" / "class_a"
    class_dir.mkdir(parents=True)
    np.savez_compressed(class_dir / "sample.npz", image=np.ones((2, 4, 4), dtype=np.float32))
    ...
    assert batch["image"].shape == (1, 2, 4, 4)


def test_create_data_loader_routes_array_folder(tmp_path):
    """The generic array_folder dataset name should use the local array loader."""
    class_dir = tmp_path / "train" / "class_a"
    class_dir.mkdir(parents=True)
    ...
    loader = create_data_loader(config, split="train", batch_size=1)
    batch = next(iter(loader))
