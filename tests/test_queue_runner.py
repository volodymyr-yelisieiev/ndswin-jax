...
def test_build_command_fetch_data():
    """Test building fetch-data queue commands."""
    from ndswin.cli import build_command

    job = {
        "type": "fetch-data",
        "hf_id": "jxie/modelnet40",
        "outdir": "data/modelnet40",
        "limit": 10,
    }
    cmd = build_command(job, "python")

    assert cmd[:3] == ["python", "-m", "ndswin.cli"]
    assert "fetch-data" in cmd
    assert "--hf-id" in cmd
    assert "jxie/modelnet40" in cmd
    assert "--outdir" in cmd
    assert "data/modelnet40" in cmd
    assert "--limit" in cmd
    assert "10" in cmd
...
def test_build_command_missing_fields():
    ...
    with pytest.raises(ValueError, match="requires 'hf_id'"):
        build_command({"type": "fetch-data", "outdir": "data/x"}, "python")

    with pytest.raises(ValueError, match="requires 'outdir'"):
        build_command({"type": "fetch-data", "hf_id": "demo/data"}, "python")
...
@pytest.mark.skipif(not HAS_YAML, reason="PyYAML not installed")
def test_queue_runner_writes_configurable_results_path(tmp_path: Path):
    """Dry-run queue results should be writable outside logs/ for isolated tests."""
    from train.queue_runner import load_completed_jobs
        ...
        "jobs": [
            {
                ...
                "type": "fetch-data",
                "hf_id": "demo/dataset",
                "outdir": str(tmp_path / "data"),
            }
        ]
    }
    queue_path = tmp_path / "queue.yaml"
    results_path = tmp_path / "queue_results.json"
    queue_path.write_text(yaml.dump(queue_data))
    ...
    assert payload[0]["name"] == "fetch_demo"
    assert payload[0]["status"] == "dry"
