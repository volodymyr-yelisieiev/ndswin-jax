...
class TestStampNaming:
    ...
    def test_no_double_hf_prefix(self):
        ...
        exp = ExperimentConfig(name="cifar100")
        stamp = exp.get_stamp()
        # Should NOT contain "hf" at all since name is "cifar100"
        assert "hf_hf" not in stamp
        # Should start with cifar100
        assert stamp.startswith("cifar100_")

    def test_hf_prefix_stripped(self):
        from ndswin.config import ExperimentConfig
        exp = ExperimentConfig(name="hf_cifar100")
        stamp = exp.get_stamp()
        # Should strip the hf_ prefix
        assert stamp.startswith("cifar100_")
...
class TestCifar100DataRange:
    ...
    def test_cifar100_loader_normalized_range(self):
        """Verify CIFAR-100 data is properly normalized (not raw 0-255)."""
        from ndswin.training.data import CIFAR100DataLoader
        try:
