...
def test_classification_generalization(rng):
    ...
    train_config = TrainingConfig(
        batch_size=batch_size, learning_rate=1e-3, num_classes=num_classes, loss="cross_entropy"
    )
...
def test_segmentation_generalization(rng):
    ...
    train_config = TrainingConfig(
        batch_size=batch_size, learning_rate=1e-3, num_classes=num_classes, loss="dice"
    )

    # Use SwinForSegmentation for segmentation tasks
    model = SwinForSegmentation(config=model_config, num_classes=num_classes)
...
def test_bce_segmentation_broadcasting(rng):
    ...
    train_config = TrainingConfig(
        batch_size=batch_size, learning_rate=1e-3, num_classes=num_classes, loss="bce"
    )
